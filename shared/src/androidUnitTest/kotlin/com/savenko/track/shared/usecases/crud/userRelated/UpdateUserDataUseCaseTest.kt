    ...
    fun `new values are inserted correctly`() = runTest {
        when (val paramValue = updateUserDataUseCaseParam.param) {
            is UpdateUserDataUseCaseParam.UserName -> {
                ...
                Mockito.`when`(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager.nameFlow)
                    .thenReturn(flow { emit(currentParamValue) })
                updateUserDataUseCase(key = DataStoreManager.NAME, value = currentParamValue)
                Mockito.verify(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager).setPreference(key = DataStoreManager.NAME, value = currentParamValue)
            }

            is UpdateUserDataUseCaseParam.Budget -> {
                val currentParamValue = paramValue as Float
                Mockito.`when`(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager.budgetFlow)
                    .thenReturn(flow { emit(currentParamValue) })
                updateUserDataUseCase(key = DataStoreManager.BUDGET, value = currentParamValue)
                Mockito.verify(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager).setPreference(key = DataStoreManager.BUDGET, value = currentParamValue)
            }

            is UpdateUserDataUseCaseParam.PreferableTheme -> {
                val currentParamValue = paramValue as String
                Mockito.`when`(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager.preferableTheme)
                    .thenReturn(flow { emit(currentParamValue) })
                updateUserDataUseCase(key = DataStoreManager.PREFERABLE_THEME, value = currentParamValue)
                Mockito.verify(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager)
            ...
            is UpdateUserDataUseCaseParam.ShowPageName -> {
                ...
                Mockito.`when`(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager.isShowPageName)
                    .thenReturn(flow { emit(currentParamValue) })
            ...
            is UpdateUserDataUseCaseParam.UseSystemTheme -> {
                val currentParamValue = paramValue as Boolean
                Mockito.`when`(com.savenko.track.shared.domain.usecases.crud.userRelated.UpdateUserDataUseCaseTest.Companion.dataStoreManager.useSystemTheme)
                    .thenReturn(flow { emit(currentParamValue) })
