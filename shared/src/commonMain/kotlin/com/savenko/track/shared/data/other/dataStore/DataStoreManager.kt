package com.savenko.track.shared.data.other.dataStore
...
import com.savenko.track.shared.data.other.constants.BUDGET_DEFAULT
...
import com.savenko.track.shared.data.other.constants.NAME_DEFAULT
import com.savenko.track.shared.data.other.constants.NON_CATEGORY_FINANCIALS_DEFAULT
...
class DataStoreManager(private val dataStore: DataStore<Preferences>) {
    ...
    val loginCountFlow: Flow<Int> =
        dataStore.data.map { preferences -> preferences[LOGIN_COUNT] ?: LOGIN_COUNT_DEFAULT }

    suspend fun incrementLoginCount(dispatcher: CoroutineDispatcher = Dispatchers.Default) {
    ...
    val budgetFlow: Flow<Float> = dataStore.data.map { preferences -> preferences[BUDGET] ?: BUDGET_DEFAULT }

    val isShowPageName: Flow<Boolean> =
        dataStore.data.map { preferences -> preferences[SHOW_PAGE_NAME] ?: SHOW_PAGE_NAME_DEFAULT }

    val useSystemTheme: Flow<Boolean> =
        dataStore.data.map { preferences -> preferences[USE_SYSTEM_THEME] ?: USE_SYSTEM_THEME_DEFAULT }

    val preferableTheme: Flow<String> =
        dataStore.data.map { preferences -> preferences[PREFERABLE_THEME] ?: PREFERABLE_THEME_DEFAULT.name }
    val nonCategoryExpenses: Flow<Boolean> = dataStore.data.map { preferences ->
    ...
    val nonCategoryIncomes: Flow<Boolean> = dataStore.data.map { preferences ->
    ...
    suspend fun <T> setPreference(key: Preferences.Key<T>, value: T, dispatcher: CoroutineDispatcher = Dispatchers.Default) {
        withContext(dispatcher) {
