...
import com.savenko.track.shared.presentation.components.customComponents.ErrorText
...
fun BottomSheet(bottomSheetViewModel: BottomSheetViewModel) {
    ...
    val bottomSheetTitle =
        stringResource(if (isAddingExpense) Res.string.expense else Res.string.income)
    ...
    AnimatedVisibility(
        ...
        viewState.datePicked?.let { localDate ->
            BottomSheetDateButton(
                ...
                text = when {
                    viewState.todayButtonActiveState -> stringResource(Res.string.today)
                    viewState.yesterdayButtonActiveState -> stringResource(Res.string.yesterday)
