package com.savenko.track.shared.presentation.screens.screenComponents.settingsScreenRelated.personalPreferences
    ...
    Card(
        ...
        Column(
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center) {
                Text(
                    text = stringResource(Res.string.your_stats_personal_settings_screen),
                    style = MaterialTheme.typography.titleMedium
                )
            }
            if (statsState.value.allTimeExpensesCount == 0) {
                Text(
                    text = stringResource(Res.string.not_added_any_expenses_yet_personal_settings_screen),
                ...
                Text(
                    text = stringResource(
                        Res.string.you_have_already_added_expenses_worth_of,
                        statsState.value.allTimeExpensesCount,
                        if (statsState.value.preferableCurrency.type == CurrencyTypes.FIAT) {
                            statsState.value.allTimeExpensesSum.toDouble().toFormattedCurrency(FIAT_SCALE)
                        } else {
                            statsState.value.allTimeExpensesSum.toDouble().toFormattedCurrency(CRYPTO_SCALE)
                        },
            ...
            if (statsState.value.allTimeIncomesCount == 0) {
                Text(
                    text = stringResource(Res.string.not_added_any_incomes_yet_personal_settings_screen),
                    textAlign = TextAlign.Center,
                    modifier = Modifier.align(Alignment.CenterHorizontally)
                )
            } else {
                Text(
                    text = stringResource(
                        Res.string.you_have_already_added_incomes_worth_of,
                        statsState.value.allTimeIncomesCount,
                        if (statsState.value.preferableCurrency.type == CurrencyTypes.FIAT) {
                            statsState.value.allTimeIncomesSum.toDouble().toFormattedCurrency(FIAT_SCALE)
                        } else {
                            statsState.value.allTimeIncomesSum.toDouble().toFormattedCurrency(CRYPTO_SCALE)
                        },
                        statsState.value.preferableCurrency.ticker
                    ), textAlign = TextAlign.Center
                )
            ...
            Text(
                text = stringResource(
                    Res.string.login_message_personal_settings_screen,
